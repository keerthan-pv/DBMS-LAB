SHOW DATABASES;
CREATE DATABASE IF NOT EXISTS Fest_Database1;
USE Fest_Database1;
...
-- TASK1 
-- 1) decrease item quantity after purchase
SELECT * FROM stall_items WHERE stall_id='S1' AND item_name='Veggie Wrap';
DELIMITER //
CREATE TRIGGER decrease_quantity_after_purchase
...
-- Test: insert a new purchase
INSERT INTO purchased VALUES('P1002','S1','Veggie Wrap',NOW(),2);

SELECT * FROM stall_items WHERE stall_id='S1' AND item_name='Veggie Wrap';

-- 2) prevent >5 units purchase
...
INSERT INTO purchased VALUES('P1003','S2','Fish Tacos',NOW(),6);

-- Task2
-- 1) get total stall sales
SELECT * FROM purchased WHERE stall_id='S1';
SELECT * FROM stall_items WHERE stall_id='S1';
DELIMITER //
CREATE PROCEDURE GetStallSales(IN input_stall_id VARCHAR(5))
BEGIN
    SELECT s.stall_id, SUM(si.price_per_unit * p.quantity) AS total_revenue
    FROM purchased p
    JOIN stall_items si 
...
CALL GetStallSales('S1');
 ...
 -- 2) register a participant for event
 SELECT * FROM registration WHERE event_id='E1';
DELIMITER //
...
-- Call
CALL RegisterParticipant('E1','P1011','R200');
SELECT * FROM registration WHERE event_id='E1';


-- TASK3
-- 1)Get event cost
SELECT event_id, event_name, price FROM event WHERE event_id='E1';
DELIMITER //
CREATE FUNCTION GetEventCost(input_event_id VARCHAR(5))
...
-- Test
SELECT GetEventCost('E1');
 
-- 2) get participant's total spending
SELECT * FROM purchased WHERE SRN='P1001';
DELIMITER //
CREATE FUNCTION GetParticipantPurchaseTotal(input_SRN VARCHAR(10))
RETURNS DECIMAL(10,2)
DETERMINISTIC
BEGIN
...
-- Test
SELECT GetParticipantPurchaseTotal('P1001');
